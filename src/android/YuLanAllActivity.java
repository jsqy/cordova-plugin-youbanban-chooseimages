...
public class YuLanAllActivity extends Activity implements OnClickListener {
	...
	private void initTest() {
		...
		hListViewAdapter = new HorizontalListViewAdapterAll(getApplicationContext());
	...
	class ImageAdapter extends PagerAdapter {
		...
		public void init() {
			...
			for (int i = 0; i < MainActivity.mImgs.size(); i++) {
				...
				// 使图片实现可以放大缩小的功能
				Glide.with(context).load(MainActivity.mImgDir.getAbsoluteFile() + "/"
					+ MainActivity.mImgs.get(i)).centerCrop().placeholder(Color.WHITE).crossFade().into(image);
